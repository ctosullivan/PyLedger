...
|---|---|
| Public function/class signatures in `hledger/parser.py`, `hledger/models.py`, `hledger/reports.py`, `hledger/cli.py` | `docs/api-spec.md` |
| Module responsibilities, data flow, import rules | `docs/architecture.md` |
| Supported/unsupported hledger journal features | `docs/hledger-compatibility.md` |
...
1. Check `docs/api-spec.md` — every public function in `hledger/` must have
   an entry. Every entry must match the current signature (name, parameters,
   return type, docstring summary).

2. Check `docs/architecture.md` — the module responsibility table must
   accurately describe what each module currently does.

3. Check `docs/hledger-compatibility.md` — any format feature the parser
...
```
[ ] Every public function in hledger/ has an api-spec.md entry
...
- A function was renamed without updating `api-spec.md`
...
- A parser feature was extended to handle a new format element without
  updating `hledger-compatibility.md`
- A module was split or merged without updating `architecture.md`
